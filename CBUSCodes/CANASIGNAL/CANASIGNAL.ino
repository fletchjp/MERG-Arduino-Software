...
#define REMOTE_SIGNAL_INPUT_EVENT 1 // 1 for remote signal input, 0 for local input
...
#if USE_SHORT_EVENTS
// Define device number for this signal - for now
// I would like this to come from the node's NVs in which case it can be configured by the user.
unsigned int device_number = 513;
#if GET_DEVICE_NOS_FROM_NVS
#define MAX_NUM_DEVICE_NOS 1
...
void setup() {
  ...
  Serial.begin (115200);
  Serial << endl << endl << F("> ** CANASIGNAL module ** ") << __FILE__ << endl;
  ...
  device_number = (config.readNV(2) << 8 ) + config.readNV(3);;
#endif
...
///////////////////////////////////////////////////////////////////////////
#if USE_SHORT_EVENTS
  Serial << F("> Using short events with device number ") << device_number << endl;
#endif
 ...
 */
    ...
    switch (op_code)
    {
         ...
         Serial << F("processing  ASRQ with index ") << index << F(", device number ") << device_number;
         Serial << F(" and event value(1) ") << evval << endl; 
         if (fromEvent.get()) {
             sendEvent(OPC_ARSON,device_number);
             Serial << F("response ON") << endl; 
         } else { 
             sendEvent(OPC_ARSOF,device_number);
